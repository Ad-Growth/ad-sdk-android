...
public class AdRequest {
    ...
    public Ad getAd(String unitId, HashMap<String, Object> options) throws AdRequestException {
        ...
        if (!params.containsKey("orientation")) params.put("orientation", ScreenHelpers.getOrientation(context));
...
//        params.put("longitude", clientAddress.getLongitude());
        ...
        try {
            JSONObject response = apiClient.get("/api/adverts/search", params);
            ...
            if (e instanceof APIIOException) {
                switch (((APIIOException) e).getStatusCode()) {
                    case 0:
                        throw new AdRequestException(AdRequestException.NETWORK_ERROR);
                    case 404:
